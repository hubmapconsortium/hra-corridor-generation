...
void parse_json(json::value const &jvalue, json::value &answer)
{
   ...
   try 
   {
         ...
         {           
            Surface_mesh corridor_mesh = create_corridor(target_organ, example_tissue, result, tolerance);
            output_corridor(corridor_mesh, rui_location_id, output_corridor_dir);
         }
         else if (result.size() == 1)
         { 
            output_corridor(target_organ[result[0].first].get_raw_mesh(), rui_location_id, output_corridor_dir);
         }
